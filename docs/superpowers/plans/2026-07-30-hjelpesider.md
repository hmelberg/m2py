...
- **Eksempler med resultat hører hjemme i de TUNGE seksjonene, ikke bare der de var lette å lage.** Målt 2026-07-30 hadde safestat ni eksempler — alle i strict-seksjonene — mens `#microdata` (1172 ord), `#lagre-dele` (813), `#ai` (437), `#nokler`, `#tillit`, `#modes` og `#hybrid` hadde null. De tolv delte blokkene hadde null eksempler til sammen. openstat hadde ett på hele siden, med `#direktiver` (649 ord) og `#api` uten en eneste kodelinje. En seksjon på flere hundre ord som aldri viser hva du får igjen, er referanse — ikke dokumentasjon. Rekkefølge: eksempler i delte blokker må inn i safestat **før** et søskenrepo porter dem, ellers portes de to ganger.
...
<section id="ruter">
  ...
  <table class="doc-table">
    ...
    <tbody>
      ...
      <tr><td><code>beregning</code></td><td>Alt som trengs står i spørsmålet — ren kode, ingen eksterne data</td><td>«hvor mange dager er det til jul?»</td></tr>
